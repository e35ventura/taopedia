...
const specialPageMetaByName = new Map(specialPages.map((meta) => [meta.name, meta] as const));

export function getSpecialPageMeta(name: string) {
  return specialPageMetaByName.get(name) ?? null;
}
...
export function categoryPageTitle(categoryName: string) {

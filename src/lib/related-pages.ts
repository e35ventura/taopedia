...
export function getRelatedPages({
  ...
  const scored: Array<{ slug: string; title: string; tags: string[]; score: number }> = [];
  for (const cand of candidates) {
    ...
    scored.push({ slug: cand, title, tags: tagSource.slice(0, 2), score });
  }
  ...
  return scored.slice(0, max).map(({ slug, title, tags }) => ({ slug, title, tags }));
}

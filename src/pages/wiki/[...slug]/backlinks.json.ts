...
export async function getStaticPaths() {
  ...
  // slug — the same source links to many articles, so computing them inside the
  // entry map below would recompute each source's stats once per article it
  // backlinks to (O(articles × backlinks)). Gather them once via the shared
  // gatherLinkStatsBySlug helper — the same single pass related.json / toc.json
  // adopted — both figures resolving their targets through the complete titleBySlug.

  ...
  const body = JSON.stringify(
    {
      ...
      // readingMinutes is the estimated reading time — the same ~200 wpm ceil
      // formula the article-page footer ("N min read") and info.json / toc.json /
      // history.json expose from wordCount, so a consumer citing the article can
      // record its reading time without recomputing it or a second fetch.

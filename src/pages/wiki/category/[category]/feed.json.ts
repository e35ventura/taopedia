...
export const GET: APIRoute = async ({ site, props }) => {
  ...
  // Mirrors the category RSS endpoint, but serializes as JSON Feed 1.1 for
  // clients that prefer JSON and keeps the same published/modified dates as
  // the site-wide JSON Feed.
  const items = pages

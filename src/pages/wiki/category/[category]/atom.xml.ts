...
export const GET: APIRoute = async ({ site, props }) => {
  ...
  // Each category Atom feed mirrors the existing per-category RSS and JSON
  // feeds: same canonical article URLs, history-derived published/updated
  // dates, summaries, and categories. Atom carries both <published> and
  // <updated>, so this route derives the same full history pair as the JSON
  // feed rather than the single lastmod the RSS feed uses.
  const items = pages

...
// ---- 0) Unit: buildArticleInfo produces the correct JSON shape -------------
{
  ...
  // Non-finite counts coerce to 0 across ALL four count fields (incomingLinks,
  // referencesCount, sectionCount, wordCount) — matching the cite.json sibling —
  // so info.json's numeric fields are never emitted as JSON null. Defaults only
  // catch `undefined`, so an explicit NaN/Infinity would otherwise leak through.

...
// ---- 1) Unit: helper + builder behavior -----------------------------------
{
  ...
  assert.equal(doc.revisionCount, 6, 'builder: revisionCount field');
...
for (const slug of articleSlugs) {
  ...
  // revisionCount is the article's commit-history length — the same figure
  // info.json / history.json / cite.json / references.json / related.json expose.
  assert.ok(Number.isInteger(doc.revisionCount) && doc.revisionCount >= 0, `${slug}: toc.json revisionCount must be a non-negative integer`);

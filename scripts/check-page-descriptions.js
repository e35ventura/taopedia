...
//
// This check reads the built dist/ HTML and verifies every page has a non-empty
// <meta name="description"> in <head>, and that article pages carry an
// article-specific description (not the site-wide default that the homepage
// uses). The default is derived from the built homepage's own description tag,
// so the comparison stays in sync without hardcoding.
...
console.log(
  `Page descriptions check passed (${articleCount} articles, ${categoryCount} categories, ${specialCount} special pages)`,
);

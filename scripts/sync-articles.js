...
const unsafeContentPatterns = [
  ...
  // ldap:// ldaps:// nfs:// dav:// davs:// are the other directory / network file-share
  // schemes the OS resolves to reach a remote host — not the browser. ldap://host is a
  // classic server-side-request / JNDI-injection vector (the Log4Shell class: a lookup
  // hands the URL to a directory client that fetches and can deserialize a remote
  // payload), and nfs://dav:// mount or fetch from an attacker-named file share — the
  // same non-http internal-service-reachable class as the smb:/afp: schemes above.
  // Article links are limited to http(s); the // authority form is required so prose is
  // never affected, and these scheme names never occur as URLs in glossary prose.
  { pattern: /\b(?:ldaps|ldap|nfs|davs|dav)\s*:\/\//i, reason: 'directory and network file-share URL schemes are not allowed in article content' },
...
const obfuscatedSchemePatterns = [
  ...
  { pattern: /(?:ldaps|ldap|nfs|davs|dav)\s*:\/\//i, reason: 'directory and network file-share URL schemes are not allowed in article content' },

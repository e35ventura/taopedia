...
const unsafeContentPatterns = [
  ...
  // git:// svn:// cvs:// are version-control protocol schemes: a clicked link launches a
  // registered VC client (Git / TortoiseSVN / TortoiseCVS) to clone or check out from the
  // attacker's server, pulling attacker-controlled code — and a malicious repo's hooks
  // (e.g. post-checkout) then execute locally, a code-execution surface. Same out-of-
  // sandbox client-launch class as the blocked sftp://ssh:// schemes; article links are
  // limited to http(s). The // authority form is required so prose about "git" is unaffected.

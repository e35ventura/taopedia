...
//
// titleBySlug = the published articles. linkGraph = each article's outbound links
// (build-linkgraph.js shape: { target }). An article is "dead-end" when it links
// OUT to no OTHER published article: a self-link, or a link to an unpublished/draft
// target, never rescues it (the same published-only, self-excluded outbound join
// getArticleReferences / references.json use).
{
  const titleBySlug = { alpha: 'Alpha', beta: 'Beta', gamma: 'Gamma', delta: 'Delta' };
  const linkGraph = {
    alpha: [{ target: 'beta' }], // links out to a published article -> not dead-end
    beta: [{ target: 'beta' }, { target: 'draft' }], // only a self-link + unpublished target -> dead-end
    gamma: [], // explicit empty outbound list -> dead-end
    // delta: absent from the link graph entirely -> dead-end
  };

  assert.equal(getArticleReferences({ slug: 'beta', linkGraph, titleBySlug }).length, 0, 'self-link + unpublished target leaves beta a dead-end');
  assert.equal(getArticleReferences({ slug: 'gamma', linkGraph, titleBySlug }).length, 0, 'an empty outbound list is a dead-end');
  ...
  // A graph where every page links out to a published article yields no dead-ends.
  assert.deepEqual(
    buildDeadEndPages({
      titleBySlug: { a: 'A', b: 'B' },
...
// Ordering: dead-ends sort by title with the shared compareTitles collation (numeric,
...
// report from the same public/data/{linkgraph,slugmap,backlinks}.json the build
// wrote and assert dist/wiki/special/deadendpages.json matches it field-for-field:
// a wrong envelope, a count/length mismatch, a page that actually HAS outbound
// references leaking in as "dead-end", a non-deterministic order, or enrichment
// that disagrees with the page's own info.json would silently mislead editors.
...
// Partition invariant: a published article either links out to >= 1 published
// article or it does not, so the dead-ends (outbound count === 0) and the pages
// with outbound references must add up to the published total — an independent
// cross-check that the dead-end set is neither over- nor under-inclusive.
const withOutbound = Object.keys(realTitleBySlug).filter(
  (slug) => getArticleReferences({ slug, linkGraph: linkgraphData, titleBySlug: realTitleBySlug }).length > 0,
).length;
assert.equal(
  expected.length + withOutbound,
...
data.pages.forEach((row, i) => {
  ...
  // The dead-end invariant: a listed page must genuinely have zero published
  // outbound references, re-derived with the same join the endpoint uses.
  assert.equal(
  ...
  assert.ok(Number.isInteger(row.incomingLinks) && row.incomingLinks >= 0, `row ${i} incomingLinks must be a non-negative integer`);
...
console.log(
  `Dead-end pages check passed (${data.pages.length} dead-end pages from the built endpoint match the link graph; dead-end + linked-out partition all ${Object.keys(realTitleBySlug).length} published articles)`,
);

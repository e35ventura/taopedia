...
// – url uses encodeURIComponent on the slug path segment (safe for external consumers)
...
// ---- 0) Unit: encodeURIComponent encodes URL-reserved characters in slugs -
// Verify that the URL construction strategy used in newpages.json.ts is
// safe for slugs that contain URL-reserved characters such as /, ?, #, or %.
{
  const sensitiveSlug = 'article/with#special?chars&100%safe';
  const encoded = encodeURIComponent(sensitiveSlug);
  assert.ok(
    !/[/?#&%]/.test(encoded.replace(/%[0-9A-F]{2}/gi, '')),
    `encodeURIComponent must eliminate bare URL-reserved characters from slugs (got ${encoded})`,
  );
  assert.doesNotThrow(
    () => new URL(`https://taopedia.org/wiki/${encoded}/`),
    'encodeURIComponent(slug) must produce a valid URL path segment',
  );
  // Verify round-trip: decoding the encoded segment returns the original slug.
  assert.equal(
    decodeURIComponent(encoded),
    sensitiveSlug,
    'decodeURIComponent(encodeURIComponent(slug)) must equal the original slug',
  );
}
...
for (const entry of doc.pages) {
  ...
  // URL must be absolute, start with the site origin, and use encodeURIComponent for the slug.
  assert.ok(
  ...
  assert.equal(
    entry.url,
    `${doc.site}/wiki/${encodeURIComponent(entry.slug)}/`,
    `newpages.json: entry url must use encodeURIComponent(slug) (${entry.slug})`,
  );

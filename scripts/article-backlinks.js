...
export const buildArticleBacklinks = ({ slug, title, origin, summary = '', categories = [], incomingLinks = 0, referencesCount = 0, revisionCount = 0, firstEdited = null, lastEdited = null, backlinks = [] }) => ({
  slug,
  title,
  ...
  backlinks: backlinks.map((link) => ({
    ...
    // The linking article's revision stats — revisionCount (commit-history
    // length) and firstEdited (original publication date) alongside lastEdited
    // (last revision) — the same per-entry stats references.json and
    // allpages.json expose, so a consumer scanning the backlink list can gauge
    // each linking page's age/activity without a per-entry fetch.

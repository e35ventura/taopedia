...
// ---- 1) Unit: builder produces the correct JSON shape ----------------------
{
  ...
  assert.equal(result.backlinks[0].incomingLinks, 0, 'builder: backlinks[0].incomingLinks equals backlinks when omitted');
  ...
  assert.equal(result.backlinks[1].incomingLinks, 0, 'builder: backlinks[1].incomingLinks equals backlinks when omitted');
...
for (const slug of articleSlugs) {
  ...
  for (const entry of doc.backlinks) {
    ...
    // already expose per entry.
    assert.equal(
      entry.incomingLinks,
      publishedInboundLinkCount(backlinksData, entry.slug, titleBySlug),
      `${slug}: every backlink entry incomingLinks must match the published inbound-link count`,
    );
    assert.equal(entry.incomingLinks, entry.backlinks, `${slug}: every backlink entry incomingLinks must equal backlinks`);
    const entryIncomingLinksInfoJson = path.join(wikiDir, entry.slug, 'info.json');
    if (fs.existsSync(entryIncomingLinksInfoJson)) {
      const entryInfo = JSON.parse(fs.readFileSync(entryIncomingLinksInfoJson, 'utf8'));
      assert.equal(
        entry.incomingLinks,
        entryInfo.incomingLinks,
        `${slug}: backlink entry ${entry.slug} incomingLinks must agree with sibling info.json`,
      );
    }

...
// ---- 1) Unit: builder produces the correct JSON shape ----------------------
{
  const result = buildArticleBacklinks({
    ...
    backlinks: [
      { slug: 'neuron', title: 'Neuron', summary: 'A node in the network.', categories: ['Mechanism'], referencesCount: 5, wordCount: 321, revisionCount: 3, firstEdited: '2024-02-01T00:00:00.000Z', lastEdited: '2024-03-02T00:00:00.000Z' },
  ...
  assert.equal(result.backlinks[0].wordCount, 321, 'builder: backlinks[0].wordCount threaded verbatim');
  ...
  for (const entry of doc.backlinks) {
    ...
    if (fs.existsSync(entryInfoJsonFile)) {
      ...
      assert.ok(
        ...
        `${slug}: every backlink entry wordCount must be a non-negative integer (got ${JSON.stringify(entry.wordCount)})`,
      );
      assert.equal(
        entry.wordCount,

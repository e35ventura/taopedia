...
//   2) Parses the built dist/wiki/special/statistics.json and validates EVERY
//      field the endpoint emits — counts, dates, the topics array ordering,
//      and the derived largestTopic — so a regression in any field fails the
//      postbuild gate (not just the on-demand unit tests).
...
// ---- 1) Unit: buildStatistics with constructed inputs ---------------------
{
  ...
  // largestTopic: Consensus (2 members) beats Wallets (1 member).
...
// newestDate — must be a valid ISO-8601 date string when articles exist.
assert.ok(
...
const parsedDate = new Date(data.newestDate);
assert.ok(
  !Number.isNaN(parsedDate.getTime()),
  `newestDate must be a valid date string (got ${JSON.stringify(data.newestDate)})`,
);

// largestTopic — must be an object with non-empty name + positive count,
// consistent with topics[0] (the highest-count topic after deterministic sort).
assert.ok(
...
assert.equal(
  data.largestTopic.name,
  data.topics[0].name,
  'largestTopic.name must match topics[0].name (the highest-count topic)',
);
assert.equal(
  data.largestTopic.count,
  data.topics[0].count,
  'largestTopic.count must match topics[0].count (the highest-count topic)',
);

// topics — array whose length matches totalTopics, deterministically ordered.

...
data.pages.forEach((row, i) => {
  ...
  for (const req of row.requestedBy) {
    ...
    assert.deepEqual(
      ...
      {
        slug: req.slug,
        title: realTitleBySlug[req.slug],
        ...articleJsonCompanionUrls(data.site, req.slug),
      },
      `requester ${req.slug} for ${row.slug} must expose the canonical article companion URLs`,
    );
  }
});

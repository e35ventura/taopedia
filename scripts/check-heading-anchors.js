...
const walk = (dir) => {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    // Skip the generated per-article sub-pages (history/backlinks/cite/info):
    // they are layout pages, not rendered Markdown, so they carry no body
    // section headings for the plugin to anchor.
...
for (const file of articleFiles) {
  ...
  // Scope to the article body: the rendered Markdown inside .mw-parser-output.
  // The page title <h1 class="firstHeading"> (before) and the trailing layout
  // sections — the "Related pages" block (<h2 id="related-pages-heading">), the
  // Topics row, the metadata footer, and the Appearance panel — live outside the
  // Markdown and carry headings the plugin never touches, so they must stay out
  // of this slice. Bound the body at whichever of those trailing sections appears
  // first; the meta footer is always present, so a bound always exists.
  const start = html.indexOf('class="mw-parser-output"');
  assert.ok(start !== -1, `${where}: no mw-parser-output found`);
  const enders = ['class="related-pages"', 'class="article-topics"', 'class="mw-article-meta"']
    .map((marker) => html.indexOf(marker, start))
    .filter((i) => i !== -1);
  assert.ok(enders.length > 0, `${where}: expected a trailing section (meta footer) to bound the body`);
  const body = html.slice(start, Math.min(...enders));
